...
export function collectPlayerData(){
  ...
  inputData[cdCount][4] = actionStateDuration;
...
function findNN(){
  ...
  for(let j = 0; j < Np; j++){
    ...
    Math.abs(player[1].phys.pos.y - inputData[j][1])/100 +
    (player[1].phys.jumpsUsed !== inputData[j][2])*100 + 
    (player[1].actionState !== inputData[j][3])*10000 + 
    (actionStateDuration !== inputData[j][4])*1000;
  ...
  if(smallestIndex.length > 1){
    ...
    console.log("multiple minimum found: " + smallestIndex.length);
  }
