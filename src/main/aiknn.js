import {player, playerType, cS} from "main/main";
import {gameSettings} from "settings";
import {activeStage} from "stages/activeStage";

const Np = 1*60*60; //1 pattern = 1 frame
const Ni = 11; //# of inputs
const No = 10; //# of outputs
const inputData = createArray(Np,Ni);
const outputData = createArray(Np,No);
const actionStateDuration = [0,0];

export function resetAI(){
  console.log("AI RESET");
  actionStateDuration[0] = 0;
  actionStateDuration[1] = 0;
}

let cdCount = 0;
export function shouldCollectData(){
  return cdCount < Np;
}

function updateStateDurations(){
  if(player[0].prevActionState !== player[0].actionState){
    actionStateDuration[0] = 0;
  }else{
    actionStateDuration[0] = actionStateDuration[0] + 1;
  }
  if(player[1].prevActionState !== player[1].actionState){
    actionStateDuration[1] = 0;
  }else{
    actionStateDuration[1] = actionStateDuration[1] + 1;
  }
}

export function collectPlayerData(i){
  updateStateDurations();

  inputData[cdCount][0] = player[0].phys.pos.x; 
  inputData[cdCount][1] = player[0].phys.pos.y; 
  inputData[cdCount][2] = player[0].phys.jumpsUsed; //number of jumps used.
  inputData[cdCount][3] = player[0].actionState;
  inputData[cdCount][4] = actionStateDuration[0];
  inputData[cdCount][5] = player[0].prevActionState;
  inputData[cdCount][6] = player[1].phys.pos.x;
  inputData[cdCount][7] = player[1].phys.pos.y;
  inputData[cdCount][8] = player[1].actionState; //opponent
  inputData[cdCount][9] = actionStateDuration[1];
  inputData[cdCount][10] = player[1].prevActionState;

  outputData[cdCount][0] = player[0].inputs.lStickAxis[0].x; // analog-stick 
  outputData[cdCount][1] = player[0].inputs.lStickAxis[0].y;
  outputData[cdCount][2] = player[0].inputs.cStickAxis[0].x; // c-stick
  outputData[cdCount][3] = player[0].inputs.cStickAxis[0].y;
  outputData[cdCount][4] = player[0].inputs.a[0]; // single buttons
  outputData[cdCount][5] = player[0].inputs.b[0];
  outputData[cdCount][6] = player[0].inputs.z[0];
  outputData[cdCount][7] = player[0].inputs.x[0] || player[0].inputs.y[0]; // X and Y combined
  outputData[cdCount][8] = player[0].inputs.l[0] || player[0].inputs.r[0]; // L and R combined
  outputData[cdCount][9] = Math.max(player[0].inputs.lAnalog[0], player[0].inputs.rAnalog[0]); // max of L/R analog inputs

  cdCount++;
}

let dumpFlag = true;
export function runAI (i){
  if(shouldCollectData()){
    //do nothing for now.
  }else{
    //data dump one time
    if(!dumpFlag){
      console.log(inputData);
      console.log(outputData);
      dumpFlag = true;
    }
    //nearest neighbors
    nnPolicy(i);
    //msDelay(10);
  }
}

function collectGameData(i){
  updateStateDurations();
  const gameData = createArray(Ni,1);
  gameData[0] = -1*player[1].phys.pos.x;
  gameData[1] = player[1].phys.pos.y;
  gameData[2] = player[1].phys.jumpsUsed;
  gameData[3] = player[1].actionState;
  gameData[4] = actionStateDuration[1];
  gameData[5] = player[1].prevActionState;
  gameData[6] = -1*player[0].phys.pos.x;
  gameData[7] = player[0].phys.pos.y;
  gameData[8] = player[0].actionState;
  gameData[9] = actionStateDuration[0];
  gameData[10] = player[0].prevActionState;
  return gameData;
}

function nnPolicy(i){
  outputNN(i,findNN(collectGameData(i)));
}

function findNN(gameData){
  const e = 0.001; //threshold e
  let smallestIndex = [-1];
  let smallestDist = Math.pow(10,10);
  for(let j = 0; j < Np; j++){ // /100, *100000, *100, *100000, *1000
    const dist = Math.abs(gameData[0]-inputData[j][0])/10 + 
                 Math.abs(gameData[1]-inputData[j][1])*100000 +
                 (gameData[2] !== inputData[j][2])*100 + 
                 (gameData[3] !== inputData[j][3])*100000 + 
                 Math.abs(gameData[4]-inputData[j][4])*1000 +
                 (gameData[5] !== inputData[j][5])*10000 + 
                 Math.abs(gameData[6]-inputData[j][6])/10 + 
                 Math.abs(gameData[7]-inputData[j][7])*100000 + 
                 (gameData[8] !== inputData[j][8])*100000 + 
                 Math.abs(gameData[9]-inputData[j][9])*1000 + 
                 (gameData[10] !== inputData[j][10])*10000;
    
    if(dist < smallestDist){
      smallestDist = dist;
      smallestIndex = [j];
    }else if(dist === smallestDist){
      //same dist as smallest dist, pick randomly between the two after this loop
      smallestIndex.push(j);
    }else if(Math.abs(dist-smallestDist) < e){ 
      //close dist as smallest dist, pick randomly between the two after this loop
      //console.log("threshold reached");
      //smallestIndex.push(j);
    }
  }

  const randIndex = Math.floor(Math.random()*smallestIndex.length);
  printDebugData(smallestIndex, randIndex, smallestDist, gameData, inputData[smallestIndex[randIndex]], outputData);
  return smallestIndex[randIndex];
}

function outputNN(i,idx){
  pushInputBuffer(i);//always do this before doing AI outputs

  player[i].inputs.lStickAxis[0].x = -1*outputData[idx][0]; //reverse x-axis analog stick
  player[i].inputs.lStickAxis[0].y = outputData[idx][1];
  player[i].inputs.cStickAxis[0].x = -1*outputData[idx][2]; //reverse c-stick
  player[i].inputs.cStickAxis[0].y = outputData[idx][3];

  player[i].inputs.a[0] = outputData[idx][4]; //single button inputs
  player[i].inputs.b[0] = outputData[idx][5];
  player[i].inputs.z[0] = outputData[idx][6];

  player[i].inputs.x[0] = outputData[idx][7]; //same function buttons 
  player[i].inputs.y[0] = outputData[idx][7];
  player[i].inputs.l[0] = outputData[idx][8];
  player[i].inputs.r[0] = outputData[idx][8];

  player[i].inputs.lAnalog[0] = outputData[idx][9]; //analog trigger inputs
  player[i].inputs.rAnalog[0] = outputData[idx][9];
}

function pushInputBuffer(i){
  for (let j = 0; j < 7; j++) {
    player[i].inputs.lStickAxis[7 - j].x = player[i].inputs.lStickAxis[6 - j].x;
    player[i].inputs.lStickAxis[7 - j].y = player[i].inputs.lStickAxis[6 - j].y;
    player[i].inputs.rawlStickAxis[7 - j].x = player[i].inputs.rawlStickAxis[6 - j].x;
    player[i].inputs.rawlStickAxis[7 - j].y = player[i].inputs.rawlStickAxis[6 - j].y;
    player[i].inputs.cStickAxis[7 - j].x = player[i].inputs.cStickAxis[6 - j].x;
    player[i].inputs.cStickAxis[7 - j].y = player[i].inputs.cStickAxis[6 - j].y;
    player[i].inputs.lAnalog[7 - j] = player[i].inputs.lAnalog[6 - j];
    player[i].inputs.rAnalog[7 - j] = player[i].inputs.rAnalog[6 - j];
    player[i].inputs.s[7 - j] = player[i].inputs.s[6 - j];
    player[i].inputs.z[7 - j] = player[i].inputs.z[6 - j];
    player[i].inputs.a[7 - j] = player[i].inputs.a[6 - j];
    player[i].inputs.b[7 - j] = player[i].inputs.b[6 - j];
    player[i].inputs.x[7 - j] = player[i].inputs.x[6 - j];
    player[i].inputs.y[7 - j] = player[i].inputs.y[6 - j];
    player[i].inputs.r[7 - j] = player[i].inputs.r[6 - j];
    player[i].inputs.l[7 - j] = player[i].inputs.l[6 - j];
    player[i].inputs.dpadleft[7 - j] = player[i].inputs.dpadleft[6 - j];
    player[i].inputs.dpaddown[7 - j] = player[i].inputs.dpaddown[6 - j];
    player[i].inputs.dpadright[7 - j] = player[i].inputs.dpadright[6 - j];
    player[i].inputs.dpadup[7 - j] = player[i].inputs.dpadup[6 - j];
  }
}

function printDebugData(smallestIndex, randIndex, smallestDist, gameData, inputData, outputData){
  console.log("Smallest Indices: " + smallestIndex + " Index Picked: " + smallestIndex[randIndex]);
  console.log("Smallest Distance: " + smallestDist);
  console.log("GData: " + Math.round(gameData[0]) + ", " + Math.round(gameData[1]) + ", " + gameData[2] + ", " + gameData[3] + ", " + gameData[4] + ", "
     + gameData[5] + ", " + Math.round(gameData[6]) + ", " + Math.round(gameData[7]) + ", " + gameData[8] + ", " + gameData[9] + ", " + gameData[10]);
  console.log("IData: " + Math.round(inputData[0]) + ", " + Math.round(inputData[1]) + ", " + inputData[2] + ", " + inputData[3] + ", " + inputData[4] + ", "
     + inputData[5] + ", " + Math.round(inputData[6]) + ", " + Math.round(inputData[7]) + ", " + inputData[8] + ", " + inputData[9] + ", " + inputData[10]);
  for(let k = 0; k < smallestIndex.length; k++){
    console.log("OData: " + outputData[smallestIndex[k]]);
  }
}




//util functions
function msDelay(ms){
  //console.log("delay is being used somewhere.");
  for(let m = 0; m < Math.round(ms); m++){
    for(let s = 0; s < 28000; s++){
      const a = Math.exp(10,2);
    }
  }
}


//lin algebra
function createArray(length){
  const arr = new Array(length || 0);
  let i = length;

  if(arguments.length > 1){
    const args = Array.prototype.slice.call(arguments,1);
    while(i--){
      arr[length-1-i] = createArray.apply(this,args);
    }
  }
  return arr;
}

function getUnique(data){
  const uniqueIndices = [];
  const uniqueVectors = [];
  for(let i = 0; i < data.length; i++){
    let vectorFound = false;
    for(let j = 0; j < uniqueVectors.length; j++){
      if(isVectorEqual(data[i], uniqueVectors[j])){
        uniqueIndices[j].push(i);
        vectorFound = true;
      }else{
      }
    }
    if(!vectorFound){
      uniqueVectors.push(data[i]);
      uniqueIndices.push([i]);
    }
  }
  return uniqueIndices;
}

function isVectorEqual(vec1, vec2){ //numerical
  let output = true;
  if(vec1.length === vec2.length){
    for(let i = 0; i < vec1.length; i++){
      output = output && (vec1[i] === vec2[i]);
    }
    return output;
  }else{
  	return false;
  }
}
